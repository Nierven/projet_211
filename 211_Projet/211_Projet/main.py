...
for compteur_k in range(1,25):  
    k = 0.001+0.001*compteur_k  
    for compteur_dx in range(1,25):
        dx = 0.31+0.01*compteur_dx
        verbose = True
        deferred_mode = True
        ...
        stop_time = 2              # Time to stop simulation  [s]
        time_speed = 1             # Time speed multiplier    [1]
        fps = 1200                   # Frames per second        [frame/s]
        ...
        def partial_derivatives(last_k, last_dx):
            pass
        #    k_derivative = last_k + last_dx - 
        #    dx_derivative = 
            
        def new_parameters(last_k, last_dx):
            [derivee_theta_0, derivee_theta_1] = partial_derivatives(last_k,last_dx)
            new_k = last_k - (learning_rate_ALPHA * k_derivative)
            new_dx = last_dx - (learning_rate_ALPHA * dx_derivative)
            return [new_k,new_dx]
        ...
        #Show velocity
        print("Wave velocity: {0}, dx: {1}, k: {3}".format(speed,dx,k)) 
        
        value_v_dx_k += [speed,dx,k]
