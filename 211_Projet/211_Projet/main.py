...
time_speed = 0.001             # Time speed multiplier    [1]
fps = 60                   # Frames per second        [frame/s]
...
def partial_derivatives(k, dx):
    ...
    new_dx = dx + step_dx
    error_dx = cost(k,dx)
    new_error_dx = cost(new_dx,dx)
    derivate_cost_dx = (new_error_dx- error_dx)/step_dx
    
    return derivate_cost_k, derivate_cost_dx
    
def new_parameters(last_k, last_dx):
    [derivate_cost_k, derivate_cost_dx] = partial_derivatives(k,dx)
    new_k =  k - (learning_rate * derivate_cost_k)
    new_dx = dx - (learning_rate * derivate_cost_dx)
    return [new_k,new_dx]
